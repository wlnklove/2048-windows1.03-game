/* 2048.c - Windows 1.03 Edition v2.1                         */
/* IM1024 256色优化 + 动态棋盘尺寸 + 柔和灰配色              */
/* 游戏逻辑完全保留 v1.1 不变                                 */
...
/* ====== IM1024 SVGA 优化模式 ====== */
int  g_bIM1024 = FALSE;       /* 检测到 IM1024 显卡 */
int  g_bSVGA   = FALSE;       /* SVGA 优化模式是否激活 */
...
{
    HDC hdc;
    int numColors, horzRes, bitsPixel, planes;
    HBITMAP hbmTest;
    BITMAP bm;
    ...
    /* 创建测试位图，检查 CreateCompatibleBitmap 实际产生的格式 */
    hbmTest = CreateCompatibleBitmap(hdc, 1, 1);
    if (hbmTest) {
        GetObject(hbmTest, sizeof(BITMAP), (LPSTR)&bm);
    ...
    /* IM1024 判定条件（组合判断）：
     *   1. NUMCOLORS >= 256（256色显示）
     *   2. HORZRES >= 1024（IM1024 分辨率）
     *   3. 兼容位图不是 1 plane / 8 bpp（否则已是最优，无需优化）
     */
    if (numColors >= 256 && horzRes >= 1024) {
...
LONG     lParam;
{
    ...
    case WM_PAINT:
        ...
        sprintf(buf, "2048  Score: %d%s", score,
                 g_bSVGA ? "  [SVGA]" : "");
        TextOut(hDC,
        ...
        if (game_over) {
            ...
            TextOut(hDC,
                    (cxClient - 29 * 8) / 2,
                    20,
                    "Arrows: Move  N: New  V: SVGA", 29);
        }

        /* 3. 游戏区域双缓冲 */
        hdcMem = CreateCompatibleDC(hDC);
        if (hdcMem != NULL) {
            /* SVGA 模式：创建 8bpp 线性位图（匹配 IM1024 显存格式）
             * 普通模式：创建兼容位图（16色 4-plane） */
            if (g_bSVGA) {
                hbmMem = CreateBitmap(boardPixels, boardPixels, 1, 8, NULL);
            } else {
    ...
    case WM_KEYDOWN:
        ...
        case 'V': case 'v':
            ...
            if (!g_bIM1024) {
                MessageBox(hWnd,
                    "Need IM1024 display cant match uwu~",
                    "SVGA Mode",
                ...
                if (g_bSVGA) {
                    MessageBox(hWnd,
                        "256c SVGA Active 8bpp linear bitmap for IM1024",
                        "SVGA Mode", MB_OK | MB_ICONASTERISK);
                } else {
                    MessageBox(hWnd,
                        "256c SVGA Off Reverted to standard bitmap",
                        "SVGA Mode", MB_OK);
                }
                InvalidateRect(hWnd, NULL, FALSE);
            }
...
{
    /* 格子颜色——保留经典 2048 暖色调 */
    hBrushes[0]  = CreateSolidBrush(RGB(160, 160, 160));  /* 空白格（介于底板和背景之间） */
    ...
    /* 柔和灰配色——替代蓝色+黑色
     * 背景：浅灰 RGB(192,192,192)——16色下纯色，不抖动
     * 底板：中灰 RGB(128,128,128)——16色下纯色，不抖动
     * 白/黑 3D 边框在灰底上清晰可见，柔和而不失立体感 */
